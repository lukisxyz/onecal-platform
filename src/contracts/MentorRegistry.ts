export const mentorRegistryAbi = [
	{
		type: "function",
		name: "ADDRESS_UPDATE_TYPEHASH",
		inputs: [],
		outputs: [{ name: "", type: "bytes32", internalType: "bytes32" }],
		stateMutability: "view",
	},
	{
		type: "function",
		name: "MENTOR_REGISTER_TYPEHASH",
		inputs: [],
		outputs: [{ name: "", type: "bytes32", internalType: "bytes32" }],
		stateMutability: "view",
	},
	{
		type: "function",
		name: "getMentor",
		inputs: [{ name: "_username", type: "string", internalType: "string" }],
		outputs: [
			{ name: "username", type: "string", internalType: "string" },
			{ name: "currentAddress", type: "address", internalType: "address" },
			{ name: "exists", type: "bool", internalType: "bool" },
		],
		stateMutability: "view",
	},
	{
		type: "function",
		name: "getMentorByAddress",
		inputs: [
			{ name: "_mentorAddress", type: "address", internalType: "address" },
		],
		outputs: [
			{ name: "username", type: "string", internalType: "string" },
			{ name: "mentorAddr", type: "address", internalType: "address" },
			{ name: "exists", type: "bool", internalType: "bool" },
		],
		stateMutability: "view",
	},
	{
		type: "function",
		name: "getNonce",
		inputs: [{ name: "_address", type: "address", internalType: "address" }],
		outputs: [{ name: "nonce", type: "uint256", internalType: "uint256" }],
		stateMutability: "view",
	},
	{
		type: "function",
		name: "mentorAddressToUsername",
		inputs: [{ name: "", type: "address", internalType: "address" }],
		outputs: [{ name: "", type: "string", internalType: "string" }],
		stateMutability: "view",
	},
	{
		type: "function",
		name: "mentors",
		inputs: [{ name: "", type: "string", internalType: "string" }],
		outputs: [
			{ name: "username", type: "string", internalType: "string" },
			{ name: "currentAddress", type: "address", internalType: "address" },
		],
		stateMutability: "view",
	},
	{
		type: "function",
		name: "nonces",
		inputs: [{ name: "", type: "address", internalType: "address" }],
		outputs: [{ name: "", type: "uint256", internalType: "uint256" }],
		stateMutability: "view",
	},
	{
		type: "function",
		name: "registerMentor",
		inputs: [
			{ name: "_username", type: "string", internalType: "string" },
			{ name: "_mentorAddress", type: "address", internalType: "address" },
		],
		outputs: [],
		stateMutability: "nonpayable",
	},
	{
		type: "function",
		name: "registerMentorByRelayer",
		inputs: [
			{ name: "_username", type: "string", internalType: "string" },
			{ name: "_mentorAddress", type: "address", internalType: "address" },
			{ name: "_deadline", type: "uint256", internalType: "uint256" },
			{ name: "_v", type: "uint8", internalType: "uint8" },
			{ name: "_r", type: "bytes32", internalType: "bytes32" },
			{ name: "_s", type: "bytes32", internalType: "bytes32" },
		],
		outputs: [],
		stateMutability: "nonpayable",
	},
	{
		type: "function",
		name: "updateAddress",
		inputs: [
			{ name: "_username", type: "string", internalType: "string" },
			{ name: "_newAddress", type: "address", internalType: "address" },
		],
		outputs: [],
		stateMutability: "nonpayable",
	},
	{
		type: "function",
		name: "updateAddressByRelayer",
		inputs: [
			{ name: "_username", type: "string", internalType: "string" },
			{ name: "_newAddress", type: "address", internalType: "address" },
			{ name: "_deadline", type: "uint256", internalType: "uint256" },
			{ name: "_v", type: "uint8", internalType: "uint8" },
			{ name: "_r", type: "bytes32", internalType: "bytes32" },
			{ name: "_s", type: "bytes32", internalType: "bytes32" },
		],
		outputs: [],
		stateMutability: "nonpayable",
	},
	{
		type: "function",
		name: "usernameExists",
		inputs: [{ name: "_username", type: "string", internalType: "string" }],
		outputs: [{ name: "exists", type: "bool", internalType: "bool" }],
		stateMutability: "view",
	},
	{
		type: "event",
		name: "AddressUpdated",
		inputs: [
			{
				name: "username",
				type: "string",
				indexed: true,
				internalType: "string",
			},
			{
				name: "oldAddress",
				type: "address",
				indexed: true,
				internalType: "address",
			},
			{
				name: "newAddress",
				type: "address",
				indexed: true,
				internalType: "address",
			},
		],
		anonymous: false,
	},
	{
		type: "event",
		name: "AddressUpdatedByRelayer",
		inputs: [
			{
				name: "username",
				type: "string",
				indexed: true,
				internalType: "string",
			},
			{
				name: "oldAddress",
				type: "address",
				indexed: true,
				internalType: "address",
			},
			{
				name: "newAddress",
				type: "address",
				indexed: true,
				internalType: "address",
			},
			{
				name: "relayer",
				type: "address",
				indexed: false,
				internalType: "address",
			},
		],
		anonymous: false,
	},
	{
		type: "event",
		name: "MentorRegistered",
		inputs: [
			{
				name: "username",
				type: "string",
				indexed: true,
				internalType: "string",
			},
			{
				name: "mentorAddress",
				type: "address",
				indexed: true,
				internalType: "address",
			},
		],
		anonymous: false,
	},
	{
		type: "event",
		name: "MentorRegisteredByRelayer",
		inputs: [
			{
				name: "username",
				type: "string",
				indexed: true,
				internalType: "string",
			},
			{
				name: "mentorAddress",
				type: "address",
				indexed: true,
				internalType: "address",
			},
			{
				name: "relayer",
				type: "address",
				indexed: false,
				internalType: "address",
			},
		],
		anonymous: false,
	},
	{
		type: "error",
		name: "AddressAlreadyExists",
		inputs: [],
	},
	{
		type: "error",
		name: "DeadlineExceeded",
		inputs: [],
	},
	{
		type: "error",
		name: "EmptyUsername",
		inputs: [],
	},
	{
		type: "error",
		name: "InvalidSignature",
		inputs: [],
	},
	{
		type: "error",
		name: "InvalidUsernameFormat",
		inputs: [],
	},
	{
		type: "error",
		name: "MentorDoesNotExist",
		inputs: [],
	},
	{
		type: "error",
		name: "NonceMismatch",
		inputs: [],
	},
	{
		type: "error",
		name: "Unauthorized",
		inputs: [],
	},
	{
		type: "error",
		name: "UsernameAlreadyExists",
		inputs: [],
	},
] as const;
