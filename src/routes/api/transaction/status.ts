				...
				try {
					...
					if (!hash && !transactionId) {
						return new Response(
							JSON.stringify({
								error:
									"Either 'hash' or 'transactionId' query parameter is required",
							}),
							{ status: 400 },
						);
					}
					...
					const transformStatus = (
						status: typeof transactionStatuses.$inferSelect,
					) => ({
						...status,
						...
						sentAt: status.sentAt
							? new Date(status.sentAt).toISOString()
							: null,
						confirmedAt: status.confirmedAt
							? new Date(status.confirmedAt).toISOString()
							: null,
						eventTimestamp: status.eventTimestamp
							? new Date(status.eventTimestamp).toISOString()
							: null,
					});
